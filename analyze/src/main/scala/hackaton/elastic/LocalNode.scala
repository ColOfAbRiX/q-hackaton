...
//
//import com.carrotsearch.randomizedtesting.{ RandomContextUtils, RandomizedContext }
...
//import org.elasticsearch.node.{ InternalSettingsPreparer, NodeConstructor }
...
//class InternalLocalNode(settings: Settings, plugins: List[Class[_ <: Plugin]], context: RandomizedContext)
//    extends NodeConstructor(
//      InternalSettingsPreparer.prepareEnvironment(settings, java.util.Collections.emptyMap(), null, null),
//      plugins,
//      false,
//    )
//    with LocalNode
...
//    RandomContextUtils.dispose(context)
//    super.close()
//
...
//    ElasticClient(JavaClient(ElasticProperties(Seq(ElasticNodeEndpoint("http", host, port, None)))))
//}
//
//object LocalNode {
//
...
//    )
//    val context: RandomizedContext = RandomContextUtils.initialise()
//
...
//      .build()
//
//    new InternalLocalNode(mergedSettings, plugins, context)
//  }
//
//  // creates a LocalNode with all settings provided by the user
